...
6. **Service Fabric** — after guest IP is known, register Machine as backend of named VIPs. Deregistered the moment that guest IP stops being live: Machine deletion, `spec.powerState: Stopped`, and `spec.powerState: Halted` all remove this Machine's backend entry from every `spec.serviceFabric.services[]` VIP before completing (finalizer removal for delete; the status patch to `Stopped`/`Halted` otherwise). This also fails closed — a real deregistration error (FluxVM unreachable, a transient error) blocks that completion and gets retried next tick, rather than leaving a dead or (once the address is DHCP-reused) misattributed backend registered against a VIP indefinitely. A VIP that's already gone is tolerated as nothing left to deregister from.
