...
- guest-agent (qemu-guest-agent) integration — FluxVM already ships the host-side QGA channel/client/REST (`ping`/`exec`/`powershell`/firewall rules); Kairon-side wiring (e.g. graceful shutdown before a hard stop) not started.
