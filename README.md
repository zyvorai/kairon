...
- **`kairon-ui`'s own REST API had no route at all for `MachineQuota`/`MachineDisruptionBudget`/`MachineSet`/`MachineInstanceType`/`MigrationPolicy`** — even though `kaironctl` and `kubectl` could always see and manage all five, an operator using only the dashboard/API had no visibility into quota usage or disruption-budget state, both first-class admission-time concerns (see [Guarding the fleet](#guarding-the-fleet)). `GET /api/v1/quotas`/`disruption-budgets`/`machinesets`/`instancetypes`/`migration-policies` now wrap the same read-only `List*Namespace` calls `kaironctl get` already uses — any-authenticated-operator, matching every other read-only cross-checkable resource. List-only, API-only, no dashboard button yet, matching the bullet above — and no create/update/delete route either; `kaironctl`/`kubectl` remain how any of the five actually get mutated.
...
- **A destination node's prepared live-migration session has no timeout of its own.** `internal/migration/server.go`'s `Prepare` reserves a real incoming-QEMU process on the target the moment the source calls it — normally released moments later by the source's own `Commit`/`Abort` on every path `reconcileMigration` can take (including every non-commit exit — blocked, unsupported mode, transfer never started, failed, cancelled — see the network-quiesce bullet above for the same "always undone" property on that half). If the *source* `kairon-node` itself crashes or is fenced between a successful `Prepare` and ever calling `Commit`/`Abort` — rather than a normal in-process error path, which is already handled — the destination's reserved incoming process has nothing left to release it: no lease, no heartbeat, no background reaper. This is a distinct, narrower gap from `NeedsRecovery` (which only ever covers an *ambiguous commit result*, not an abandoned *pre-commit* session) and isn't yet closed; a real fix needs a heartbeat or TTL added to the prepare/commit/abort protocol itself, not a one-line change, so it's named here rather than left silently undiscovered.
