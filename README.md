...
- **PVC-backed boot disks are a first cut**: one boot volume per Machine, `Filesystem`-mode `PersistentVolume`s only. `hostPath`/`local` sources resolve directly; a network-block volume now works too, but only through Kairon's own first-cut CSI driver (`csiNode.enabled`, iSCSI only, no dynamic provisioning, no CHAP on Kairon's own consumption path) — see [`docs/guides/machine-storage-csi.md`](docs/guides/machine-storage-csi.md). A PV naming any other CSI driver is still refused outright.
