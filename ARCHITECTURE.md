...
- **`kairon-ui`** (optional) — a Go backend (`internal/uiapi`) plus a small React SPA, with exactly the same standing as `kaironctl`: another Kubernetes API client, no privileged side channel, no second source of truth. Its one genuinely separate piece of state is its own operator account list (`ui.auth.users`, a Kubernetes Secret) and session/lockout bookkeeping, which is why it's documented as single-replica-by-design in [SECURITY.md](SECURITY.md).
