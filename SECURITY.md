...
Known limitations of the username/password mode, by design: no password-reset flow (an operator regenerates a hash and redeploys, the same operational model as rotating `ui.token`); session revocation, the generated default-admin password, and the login rate-limiter's counters only live on one replica (the chart runs exactly one); OIDC/SSO is a bigger, separate decision and is not implemented.
