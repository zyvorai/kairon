...
Known limitations of the username/password mode, by design: session revocation and the login rate-limiter's counters only live on one replica (the chart runs exactly one -- deliberately: distributing them would mean taking on an external store like Redis, which this project's Go-stdlib-only design avoids); OIDC/SSO is a bigger, separate decision and is not implemented.
